 ...
 * @author zhangdeming
 * @date 创建时间 2016/9/23
 * @description recyclerView线性布局的分割线
 */
public class LinearItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public LinearItemDecoration(@ColorInt int color, int lineHeight, int headMargin, int margin) {
        mDrawColor = color;
        mLineHeight = lineHeight;
        mHeadMargin = headMargin;
        mLeftMargin = margin;
        mRightMargin = margin;
        mType = TYPE_COLOR;
    }
    ...
    private void drawHorizontalByDrawable(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (i == 0) {
                final int firstRight = child.getLeft() + params.leftMargin;
                final int firstLeft = firstRight - mDrawable.getIntrinsicWidth();
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mOrientation == VERTICAL) {
            if (isFirst) {
                outRect.set(mLeftMargin, offests + mHeadMargin, mRightMargin, offests);
            } else if (isLast) {
                outRect.set(mLeftMargin, 0, mRightMargin, offests + mBottomMargin);
            } else {
                outRect.set(mLeftMargin, 0, mRightMargin, offests);
            }
        } else {
            if (isFirst) {
